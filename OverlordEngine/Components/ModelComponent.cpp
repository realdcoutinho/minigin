...
ModelComponent::ModelComponent(const std::wstring& assetFile, bool castShadows, bool isLevel):
...
void ModelComponent::Draw(const SceneContext& sceneContext)
{
	...
	if (m_IsLevel)
	{
		auto submeshes = m_pMeshFilter->GetMeshes();
		for (std::vector<SubMeshFilter>::reverse_iterator it = submeshes.rbegin(); it != submeshes.rend(); ++it)
		{
		...
		/*
		for (size_t i = { m_pMeshFilter->GetMeshes().size() - 1 }; i >= 0; --i)
		{
			SubMeshFilter pSubMesh = m_pMeshFilter->GetMeshes().at(i);
			pCurrMaterial = m_Materials[pSubMesh.id] != nullptr ? m_Materials[pSubMesh.id] : m_pDefaultMaterial;
			pCurrMaterial->UpdateEffectVariables(sceneContext, this);
			...
			//Set Vertex Buffer
			const UINT offset = 0;
			const auto& vertexBufferData = m_pMeshFilter->GetVertexBufferData(sceneContext, pCurrMaterial, pSubMesh.id);
			pDeviceContext->IASetVertexBuffers(0, 1, &vertexBufferData.pVertexBuffer, &vertexBufferData.VertexStride,
				&offset);

			//Set Index Buffer
			pDeviceContext->IASetIndexBuffer(pSubMesh.buffers.pIndexBuffer, DXGI_FORMAT_R32_UINT, 0);
			...
			{
				tech->GetPassByIndex(p)->Apply(0, pDeviceContext);
				pDeviceContext->DrawIndexed(pSubMesh.indexCount, 0, 0);
			}
		}
		*/
	}
...
void ModelComponent::ShadowMapDraw(const SceneContext& sceneContext)
{
	...
	bool isSkinned = m_pMeshFilter->HasAnimations();
	...
	ShadowMapRenderer* shadowMapRenderer = ShadowMapRenderer::Get();

		if (isSkinned && m_enableShadowMapDraw) 
		{
