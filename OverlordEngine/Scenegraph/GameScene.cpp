...
#pragma region POST-PROCESSING PASS
	...
	if (m_PostProcessingMaterials.size() > 0)
	{
		...
		for (auto pPostMaterial : m_PostProcessingMaterials)
		{
			if (!pPostMaterial->IsEnabled())
			{
				continue;
			}

			pPostMaterial->Draw(m_SceneContext, pPrevRenderTarget);
			pPrevRenderTarget = pPostMaterial->GetOutput();
		}
		...
		if (pPrevRenderTarget != pInitRenderTarget)
		{
			...
			SpriteRenderer::Get()->DrawImmediate(m_SceneContext.d3dContext, pPrevRenderTarget->GetColorShaderResourceView(), DirectX::XMFLOAT2{});
			//return;
		}
		//return;
		//else
